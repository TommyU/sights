...
function _M:get_downloaded_list()
    local sql = [[select id, video_name, video_size, ctime, last_downloaded_time, downloaded_times, keyword, youtube_url_hash  as hash, is_deleted, deleted_time, need2sync, is_synced from sights.video_tab where is_deleted=0 order by id desc ]]
    local res = assert(mysql_client:query(sql))
    return res
end
